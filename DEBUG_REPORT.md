...
# 第八次调试报告：抬手亮屏 + 通知页面 + Deep sleep（2026-05-21）

## 概述

在 BLE/WiFi 通信就绪基础上，补齐续航相关三大功能：抬手亮屏、通知 UI、Deep sleep。

## 完成内容

| 功能 | 实现 |
|------|------|
| 抬手亮屏 | IMU 重力矢量低通滤波 → 检测重力方向突变 → 判定抬腕 → 自动亮背光 |
| 息屏定时 | 最后活动后 10s 自动关背光；触摸/抬腕重置定时器 |
| 通知 UI | 第 4 页显示 BLE 通知（App/标题/正文），`LV_LABEL_LONG_WRAP` 自动换行 |
| Deep sleep | 息屏 30s 后进入深度睡眠；触摸中断 (INT=13) 或 RTC 定时 (60s) 唤醒 |

## 关键设计

### 抬手亮屏算法（v2 — 修复版）

**v1 bug**：比较前后帧低通滤波重力 `|gravₜ - gravₜ₋₁|`，每帧最大变化仅 `0.05 × 2g = 0.1g`，永远达不到 0.35 阈值，抬腕检测实际未生效。

**v2 修复**：

```
acc → 低通滤波 (α=0.02, ~0.5s) → gravity vector
  → 静止时建立休止基线 rest_grav (motion < 0.03 且持续 > 100 帧)
  → 实时计算 gravity 与 rest_grav 夹角 angle
  → motion > 0.08 且 angle > 25° → 判定抬腕
  → 3s 后自动清除
```
...
| 状态 | 触发条件 | 背光 | 下一状态 |
|------|---------|------|---------|
| 活跃 | 触摸/抬腕/刚启动 | ON | 10s 无活动 → 息屏 |
| 息屏 | 10s 无活动 | OFF | 触摸/抬腕 → 活跃；30s → Deep sleep |
| Deep sleep | 30s 无活动 | OFF | 触摸中断或 60s RTC 定时唤醒 → 重启 |

### Deep sleep 配置

- `esp_sleep_enable_timer_wakeup(60000000)` — 60 秒 RTC 定时唤醒
- `esp_sleep_enable_ext0_wakeup(GPIO13, 0)` — CST816D INT 引脚低电平唤醒（触摸检测）
- 睡前关闭 AXP2101 DC1/ALDO1 及所有 IRQ

## 文件变更

| 文件 | 变更 |
|------|------|
| `src/main.cpp` | + 抬手亮屏函数、背光控制、活动定时器、第 4 通知页、Deep sleep 入口 |
| `CLAUDE.md` | 更新状态 → 抬手亮屏/Deep sleep 标记 ✅ |
...
---
### 第 8 阶段：顶部花屏修复 + 电池诊断 + 充电保活

**问题**：
1. 顶部状态栏花屏 — ST7789 物理 240x320，LVGL 用 240x284（offset_y=20），物理 0-19 行从未写入显示随机噪点
2. 电池显示 0mV, -1% — `isBatteryConnect()` 返回 false，电压/百分比直接跳过读取
3. 深睡断开 USB 串口 — 无法边充电边监控

**修复**：

| 问题 | 修改 |
|------|------|
| 顶部花屏 | `gfx->begin()` 后通过 SPI 总线直接写黑像素到物理 0-19 行和 304-319 行 |
| 状态栏不存在 | `status_bar_create(lv_layer_top())` — 状态栏建在顶层图层，跨页面持久显示 |
| 电池读数为 0 | 添加 `read_batt_voltage_raw()` / `read_batt_percent_raw()` 直接读 ADC 寄存器 (0x34-0x35) 和百分寄存器 (0xA4)，绕过 `isBatteryConnect()` 检查 |
| 电池检测 | 添加 `enableBattDetection()`, `enableGauge()`, `fuelGaugeControl()`, 充电参数配置 (4.35V/500mA) |
| 深睡断串口 | 深睡前检查 `pmu.isVbusIn()`，USB 插着时跳过深睡（只息屏） |

**诊断结论**：

串口输出 `batt_adc=2mV connected=0` 且电池之前能供电（用户确认），说明 AXP2101 电池检测正常，但电池保护板已触发（过放断开）。表现为：
- 拔 USB 立即断电（电池内部 FET 断开）
- ADC 读数为浮空噪声（0-2000mV 波动）
- 无法通过 PMU 充电（检测不到电池物理存在）

**当前状态**：
- 电池保护板已锁死，需要物理断开重接或激活
- 软件层面已做完全兼容：电压/百分比直接读 ADC，不依赖检测位
- 插 USB 时不会深睡，串口保持连接
...
| 文件 | 变更 |
|------|------|
| `src/main.cpp` | + 清除 ST7789 越界像素、状态栏顶层图层、原始 ADC 读取函数、PMU gauge/充电配置、深睡 USB 保护；改充电检测逻辑 |
| `DEBUG_REPORT.md` | 追加第 8 阶段报告 |
...
1. Deep sleep 后全部 RAM 丢失（包括 LVGL 状态），唤醒后从 `setup()` 完整重初始化
2. 抬腕阈值 `0.35g` 为初步值，需实测调整防误触
