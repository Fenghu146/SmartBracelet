...
**当前阶段目标**：补齐通信 (BLE/WiFi) + 续航优化 (Deep sleep)，达到可日常佩戴的基础体验。
...
| 功能       | 类别  | 优先级 | 状态  | 说明                  |
| -------- | --- | --- | --- | ------------------- |
| 时间显示     | 核心  | P0  | ✅   | LVGL 数字表盘 + RTC 校时  |
| 触摸交互     | 核心  | P0  | ✅   | LVGL 滑动/点击/手势翻页     |
| 背光控制     | 核心  | P0  | ⏳   | 已点亮，待调节 + 自动息屏      |
| 低功耗休眠    | 核心  | P0  | ❌   | Deep sleep + RTC 唤醒 |
| 计步器      | 健康  | P0  | ✅   | IMU 峰值检测算法          |
| 通知同步     | 通信  | P0  | ❌   | BLE GATT，当前重点       |
| WiFi 校时  | 通信  | P0  | ❌   | NTP → RTC，当前重点      |
| 抬手亮屏     | 交互  | P1  | ❌   | IMU 抬腕检测            |
| 天气显示     | 工具  | P1  | ❌   | WiFi 获取             |
| 秒表/倒计时   | 工具  | P1  | ❌   | 基础功能                |
...
| BLE    | **NimBLE-Arduino** (计划)   | 比 Bluedroid 省 ~50KB RAM |
...
| RTC PCF85063             | ✅   | 时间读写, WiFi 校时就绪                          |
| IMU QMI8658              | ✅   | 加速度+陀螺仪, 计步算法                            |
| LVGL 表盘 UI               | ✅   | 数字时钟/日期/电池/步数/2 页导航                      |
| BLE 连接                   | ❌   | **当前任务**                                 |
| WiFi NTP 校时              | ❌   | **当前任务**                                 |

---

### 第一阶段：通信 + 续航（Phase 2 — 4 周）

**目标**：补齐 BLE 通知同步 + WiFi 校时 + 低功耗深度睡眠，达到可日常佩戴的基础体验。

| 周次  | 任务                                                                                   | 交付物                  |
| --- | ------------------------------------------------------------------------------------ | -------------------- |
| 1   | **BLE 基础服务**：集成 NimBLE，实现 GATT Server (Device Info / Battery / Current Time Service) | nRF Connect 可发现并连接手表 |
| 2   | **WiFi 校时 + 通知通道**：WiFi 管理器 + NTP → RTC；定义 Notification GATT Profile                 | 手机 App 推送通知显示在手表     |
| 3   | **通知 UI + 抬手亮屏**：通知列表页面；IMU 抬腕检测 + 背光自动管理                                            | 收到通知可查看，抬手即亮         |
| 4   | **Deep Sleep 低功耗**：AXP2101 电源状态机 (活跃→空闲→浅睡→深睡)；RTC/触摸/IMU 唤醒                         | 实测各模式功耗              |

**关键技术决策：**

| 决策      | 选项                 | 选择                  | 原因                   |
| ------- | ------------------ | ------------------- | -------------------- |
| BLE 协议栈 | Bluedroid / NimBLE | **NimBLE**          | 省 ~50KB RAM，低功耗更优    |
| 通知协议    | ANCS / 自定义 GATT    | **NimBLE 自定义 GATT** | 兼容 Android，后续支持 ANCS |
| 时间同步    | BLE / WiFi NTP     | **WiFi NTP**        | 无需手机 App，首次配网后自动同步   |
| 唤醒源     | RTC / 触摸 / IMU     | **三源组合**            | RTC 定时测温，触摸亮屏，IMU 抬手 |
| 配网方式    | AP 配网 / BLE 配网     | **BLE 配网**          | 手机 App 统一管理，体验更好     |
...
| 周次  | 任务                                  | 交付物           |
| --- | ----------------------------------- | ------------- |
| 5   | **OTA 升级**：HTTP 固件下载 + 差分更新         | 无线固件更新        |
| 6   | **天气 + 日历**：WiFi 获取天气 API，LVGL 天气页面 | 实时天气显示        |
| 7   | **秒表/倒计时 + BLE 音乐控制**               | 工具功能 + HID 遥控 |
| 8   | **综合调试 + 续航优化**：各模式电流测量，代码尺寸优化      | 稳定版发布         |
...
| 周次  | 任务                          | 交付物       |
| --- | --------------------------- | --------- |
| 9   | TFLite Micro 集成 + ESP-NN 加速 | 可运行的推理管线  |
| 10  | 活动识别模型（行走/跑步/静止）            | 自动活动检测    |
| 11  | ESP-SR 离线语音唤醒               | "嘿 手表" 唤醒 |
| 12  | 异常检测（跌倒检测）                  | 安全告警      |
...
| IRAM 溢出（TG0WDT 复位）            | 高   | 低   | 已解决：精简 lib_deps（仅 CST816S + lvgl），IRAM 从 60KB 降至安全范围            |
| NimBLE + LVGL + WiFi 共存内存不足   | 中   | 中   | 当前 RAM 101KB/320KB (30.8%)，NimBLE 预估 +50KB，仍有 ~170KB 余量         |
| BLE + WiFi 共存射频干扰             | 中   | 低   | 分时复用，连接 WiFi 时暂停 BLE 广播                                         |
| AXP2101 Deep sleep 配置错误导致无法唤醒 | 高   | 中   | 先验证触摸唤醒，再验证 RTC 定时唤醒，逐步推进                                       |
...
| 无 TFLite（需安装）影响 AI 进度         | 中   | 高   | Phase 3 才开始，有充足时间准备                                             |
...
见 `DEBUG_REPORT.md` — 包含以下 6 次调试记录：
