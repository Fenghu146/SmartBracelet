...
class NotifyCallback : public BLECharacteristicCallbacks
{
    void onWrite(BLECharacteristic *chr)
    {
        ...
        // Location command protocol: "loc:lat|lon"
        if (val.rfind("loc:", 0) == 0) {
            std::string coords = val.substr(4);
            int sep = coords.find('|');
            if (sep > 0) {
                std::string lat = coords.substr(0, sep);
                std::string lon = coords.substr(sep + 1);
                nvs_set_weather_lat(lat.c_str());
                nvs_set_weather_lon(lon.c_str());
                LOG_INFO("BLE: location saved lat=%s lon=%s", lat.c_str(), lon.c_str());
            }
            return;
        }
